...
function AnimeDetail() {
  ...
  const { watchlist, addToWatchlist, removeFromWatchlist, saving } = useWatchlist();
  ...
  // Derive isInWatchlist from shared context
  const isInWatchlist = useMemo(() => {
    if (!anime || !watchlist.length) return false;
    return watchlist.some((w) => String(w.id) === String(anime.id));
  }, [anime, watchlist]);
  ...
  useEffect(() => {
    ...
    async function fetchEpisodes() {
      ...
      try {
        ...
        // Handle pagination if needed — first page only for now
  ...
  // Add/remove from watchlist via shared context
